...
public class TestWorkoutHandler implements iWorkoutHandler {
    ...
    public List<Workout> getAllOfType(Type type) {
        List<Workout> returnWorkouts = new ArrayList<>();
        for (Workout workout : workouts) {
            if (workout.getType() == type)
                returnWorkouts.add(workout);
        }
    ...
    public Workout update(Workout workout) {
        for (Workout _workout : workouts) {
            if (_workout.getId() == workout.getId())
                _workout.setName(workout.getName());
            _workout.setDescription(workout.getDescription());
            _workout.setCategory(workout.getCategory());
            _workout.setLevel(workout.getLevel());
